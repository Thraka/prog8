...
class Antlr2KotlinVisitorQB(val source: SourceCode): AbstractParseTreeVisitor<Node>(), Prog8QBVisitor<Node> {
    ...
    override fun visitExpression(ctx: ExpressionContext): Expression {
        ...
        if(ctx.sizeof_expression!=null) {
            // Handle pointer type argument: sizeof(PTR UBYTE) or sizeof(^^UBYTE)
            ...
            // Handle basedatatype argument: sizeof(UBYTE)
            ...
            // Handle scoped_identifier argument: sizeof(x) or sizeof(MyStruct)
            ...
            // Handle ADDRESSOF(x) / TYPEDADDR(x) argument forms.
            val addressofExpr = ctx.sizeof_argument().addressof_expr()
            if(addressofExpr != null) {
                val identifier = addressofExpr.scoped_identifier().accept(this) as IdentifierReference
                val index = addressofExpr.arrayindex()?.accept(this) as? ArrayIndex
                val typed = addressofExpr.TYPEDADDR_KW() != null
                val addressof = if(index != null) {
    ...
    private fun subParam(pctx: Sub_paramContext): SubroutineParameter {
        ...
        val arrayIndices = pctx.arrayindex()
        if(pctx.EMPTYARRAYSIG()!=null || arrayIndices.isNotEmpty()) {
            if(arrayIndices.size > 1)
                throw SyntaxError("2D arrays cannot be used as subroutine parameters", pctx.toPosition())
